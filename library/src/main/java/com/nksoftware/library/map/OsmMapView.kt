...
class OsmMapView(
   ...
   init {
      ...
      File("${dir}/files/maps").listFiles(fileFilter)?.forEach {
         ...
         map.chartTypes[name] = TileSourceFactory.DEFAULT_TILE_SOURCE
      }

      setOsmChart(map.chart)
      addMapListener(this)
   }
   ...
   fun update(gps: Boolean, location: ExtendedLocation, centerMap: Boolean, snackBar: (String) -> Unit) {
      try {
         ...
         setOpenseaMapOverlay(map.openSeaMap)

         if (map.chart != map.actualChart) {
            setOsmChart(map.chart)
            map.actualChart = map.chart
         }
   ...
   fun setOsmChart(chart: Int) {
      when (chart) {
         0, 1 -> {
            ...
            setTileSource(map.chartTypes[map.chartTypes.keys.toList()[chart]])
         }

         else -> {
            val tp = OfflineTileProvider(
               SimpleRegisterReceiver(context),
               arrayOf(File("${dir}/files/maps/${map.chartTypes.keys.toList()[chart]}.mbtiles"))
            )
            tileProvider = tp
